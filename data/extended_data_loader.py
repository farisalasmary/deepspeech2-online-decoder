...
def load_audio_from_txt(signal_txt, sample_rate=44100):
    # convert signal from text to numpy array
    buffer_array_txt = signal_txt.strip().split(',')
    sound = np.array(buffer_array_txt)
    sound = sound.astype('float32') / 32767  # normalize audio
    if len(sound.shape) > 1:
        if sound.shape[1] == 1:
            sound = sound.squeeze()
    ...
    sound = librosa.core.resample(sound, orig_sr=sample_rate, target_sr=16000, res_type='kaiser_fast')
    return sound

"""
def load_audio(path, is_signal_txt=False):
    if is_signal_txt:
        # convert signal from text to numpy array
        buffer_array_txt = path.strip().split(',')
        sound = np.array(buffer_array_txt)
    else:
        sample_rate, sound = read(path)
    ...
    sound = librosa.core.resample(sound, orig_sr=44100, target_sr=16000, res_type='kaiser_fast')
    return sound
"""
...
class OnlineSpectrogramParser:
    ...
    def parse_audio(self, signal_txt):
        y = load_audio_from_txt(signal_txt, sample_rate=44100)   
        if self.noiseInjector:
            add_noise = np.random.binomial(1, self.noise_prob)
            if add_noise:
                y = self.noiseInjector.inject_noise(y)
